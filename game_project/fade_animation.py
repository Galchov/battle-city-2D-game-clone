...
class Fade:
    def __init__(self, game, assets, speed=5):
        ...
        self.top_rect = pygame.Rect(0, 0 - gs.SCREEN_HEIGHT // 2, gs.SCREEN_WIDTH, gs.SCREEN_HEIGHT // 2)
        self.top_rect_start_y = 0 - gs.SCREEN_HEIGHT // 2
        self.top_rect_end_y = gs.SCREEN_HEIGHT // 2
        self.top_y = self.top_rect.bottom

        self.bottom_rect = pygame.Rect(0, gs.SCREEN_HEIGHT, gs.SCREEN_WIDTH, gs.SCREEN_HEIGHT // 2)
        self.bottom_rect_start_y = self.bottom_rect.top
        self.bottom_rect_end_y = gs.SCREEN_HEIGHT // 2
        ...
        self.stage_image_rect = self.stage_image.get_rect(center=(gs.SCREEN_WIDTH // 2, gs.SCREEN_HEIGHT // 2))
    ...
    def draw(self, window):
        pygame.draw.rect(window, gs.GREY, self.top_rect)
        pygame.draw.rect(window, gs.GREY, self.bottom_rect)
    ...
    def create_stage_image(self):
        ...
        surface.fill(gs.GREY)
        surface.blit(self.images["stage"], (0, 0))
        if self.level < 10:
            surface.blit(self.images["num_0"], (self.stage_pic_width + self.num_pic_width, 0))
        else:
            surface.blit(self.images[f"num_{str(self.level)[0]}"], (self.stage_pic_width + self.num_pic_width))
